  ...
  useEffect(() => {
    ...
    if (todayRecords.length > 0) {
      ...
      setTodayProgress({
        date: today,
        walking: {
          completed: todayRecords.reduce((sum: number, r: ExerciseRecord) => sum + (r.walking?.duration || 0), 0),
          target: goals.walking?.targetDuration || 30,
          achieved: false,
        },
        fistRaise: {
          completedSets: todayRecords.reduce((sum: number, r: ExerciseRecord) => sum + (r.fistRaise?.sets || 0), 0),
          completedReps: todayRecords.reduce((sum: number, r: ExerciseRecord) => sum + (r.fistRaise?.reps || 0), 0),
          targetSets: goals.fistRaise?.targetSets || 3,
          targetReps: goals.fistRaise?.targetReps || 10,
          achieved: false,
        },
        eyeGaze: {
          completed: todayRecords.reduce((sum: number, r: ExerciseRecord) => sum + (r.eyeGaze?.count || 0), 0),
